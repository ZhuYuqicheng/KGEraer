...
class KGDataset(object):
    ...
    def __init__(self, data_path, debug):
        ...
        self.deleted_triples = []

    def get_examples(self, split, rel_idx=-1, n_del=0, del_triple="None"):
        ...
        Args:
            ...
            n_del: the nuber of randomly deleted triples
            del_triple: torch.tensor of a specific triple to be deleted from training data
        ...
        if split == "train":
            # create r^-1 to augment training data
            if n_del != 0:
                random_indices = np.random.choice(examples.shape[0], size=n_del, replace=False)
                self.deleted_triples = examples[random_indices]
                examples = np.delete(examples, random_indices, axis=0)
            if del_triple != "None":
                del_index = np.isin(examples, np.array(del_triple)).all(axis=1)
                examples = examples[~del_index]
                self.deleted_triples = np.array(del_triple)
