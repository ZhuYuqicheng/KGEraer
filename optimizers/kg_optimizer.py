...
class KGOptimizer(object):
    ...
    def __init__(
            self, model, regularizer, optimizer, batch_size, neg_sample_size, double_neg, device, verbose=True, deleted_triple="None"):
        """Inits KGOptimizer."""
        self.model = model
        ...
        self.deleted_triple = deleted_triple
        self.device = device
    ...
    def get_neg_samples(self, input_batch):
        ...
        # new added
        if self.deleted_triple != "None":
            if not isinstance(self.deleted_triple, torch.Tensor):
                self.deleted_triple = torch.Tensor(self.deleted_triple)
            add_negative_batch = self.deleted_triple.repeat(batch_size, 1).to(
                device=input_batch.device, dtype=input_batch.dtype)
            negative_batch = torch.cat((negative_batch, add_negative_batch), dim=0)
    ...
    # new added
    def calculate_acc_valid_loss(self, examples):
        """Compute accumulate KG embedding loss over validation examples.
    ...
    def epoch(self, examples):
        ...
        """
        actual_examples = examples[torch.randperm(examples.shape[0]), :] # data shuffle
    ...
    def restrict_epoch(self, examples, freeze_index, fix_rel=False):
        """Runs one epoch of training KG embedding model.

        Args:
            examples: torch.LongTensor of shape (N_train x 3) with training triples
            mode(str): can be "relation" or "entity" to specify which embedding should be freezed
        Returns:
            loss: torch.Tensor with loss averaged over all training examples
        """
        # freeze relation embedding
        if fix_rel:
            self.model.rel.weight.requires_grad = False
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while b_begin < examples.shape[0]:
                ...
                self.optimizer.zero_grad()
                l.backward()
                # freeze part of entity embeddings
                if len(freeze_index) > 0:
                    self.model.entity.weight.grad[freeze_index] = 0
