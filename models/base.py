...
class KGModel(nn.Module, ABC):
    ...
    def __init__(self, sizes, rank, dropout, gamma, data_type, bias, init_size, device):
        """Initialize KGModel."""
    ...
    def compute_unlearning_metrics(self, training_examples, filters, deleted_triples, batch_size=500):
        ...
        Returns:
            Evaluation metrics (mean rank, mean reciprocical rank and hits)
        """
        # evaluate positive and negative rank
        pos_ranks = self.get_ranking(training_examples, {**filters["lhs"], **filters["rhs"]}, batch_size=batch_size)
        neg_ranks = self.get_ranking(deleted_triples, {**filters["lhs"], **filters["rhs"]}, batch_size=batch_size)
        # evaluate plausibility
        sig = nn.Sigmoid()
        b_begin = 0
        pos_plausibility = torch.empty(0).to(self.device)
        with torch.no_grad():
            ...
            while b_begin < training_examples.shape[0]:
                ...
                b_begin += batch_size
                positive_score, _ = self.forward(input_batch)
                pos_plausibility = torch.cat((pos_plausibility, sig(positive_score)), 0)
            # get negative plausibilities
            negative_score, _ = self.forward(deleted_triples.to(self.device))
            neg_plausibility = sig(negative_score)

        return pos_ranks, neg_ranks, pos_plausibility, neg_plausibility